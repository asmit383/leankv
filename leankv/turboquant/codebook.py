...
def _mse_cost(centroids: np.ndarray, d: int) -> float:
    ...
    for i in range(n):
        ...
        val, _ = integrate.quad(
            lambda x, c=c: (x - c) ** 2 * beta_pdf(np.array([x]), d)[0], lo, hi
        )
        cost += val
    return cost
...
def compute_lloyd_max_codebook(
    ...
    Args:
        d: dimension (e.g., head_dim = 128).
        bits: number of bits per coordinate (1-4).
    ...
    for _ in range(max_iter):
        ...
        for i in range(n_clusters):
            new_centroids[i] = _conditional_mean(
...
def get_codebook(d: int, bits: int) -> dict:
    ...
    print(
        f"[TurboQuant] MSE per coord = {cb['mse_per_coord']:.6e}, "
        f"total MSE = {cb['mse_total']:.6f}"
    )
    _CODEBOOK_CACHE[key] = cb
    return cb
